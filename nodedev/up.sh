#!/bin/bash
GOOS=linux GOARCH=arm64 go build -o nodedev/ptpnat cmd/tun/main.go
chmod +x nodedev/ptpnat

hcloud server delete nodedev-yannic
hcloud server create --name nodedev-yannic --type cax21 --image debian-12 --ssh-key yannic-mac-work
